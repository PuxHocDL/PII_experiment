...
from validate.ensembler import BoundaryAwareEnsembler
...
def ensemble_evaluate(dataset_name, evaluator, api=None, gap_threshold=0.98):
    """
    Run Token-Primary Asymmetric Ensemble with Selective Gap-Filling
    on saved evaluation predictions (BERT + DeBERTa).
    """
    debug = config.DEBUG_MODE
    seeds = [42] if debug else [42, 123, 456, 789, 999]
    safe_ds = dataset_name.split('/')[-1]
    ensembler = BoundaryAwareEnsembler()

    # Ensemble pairs: (token_model_dir, ensemble_name)
    pairs = [
        ("base_bert", "BERT Softmax + DeBERTa (Proposed)"),
        ("bert_plus_crf", "BERT+CRF + DeBERTa (Proposed)"),
    ]
    span_dir = f"./outputs/{safe_ds}/evaluations/span-based_deberta"

    if not os.path.isdir(span_dir):
        logger.warning(f"Span-based predictions not found at {span_dir}, skipping ensemble.")
        return

    for token_dir_name, ensemble_name in pairs:
        token_dir = f"./outputs/{safe_ds}/evaluations/{token_dir_name}"
        if not os.path.isdir(token_dir):
            logger.info(f"Token predictions not found at {token_dir}, skipping {ensemble_name}.")
            continue

        logger.info(f"--- Ensemble: {ensemble_name} on {safe_ds} ---")
        safe_ens_name = ensemble_name.replace(' ', '_').replace('+', 'plus').replace('/', '_').replace('(', '').replace(')', '').lower()
        out_dir = f"./outputs/{safe_ds}/evaluations/{safe_ens_name}"
        os.makedirs(out_dir, exist_ok=True)

        all_metrics = {"w_pre": [], "w_rec": [], "w_f1": []}

        for seed in seeds:
            token_file = f"{token_dir}/seed_{seed}_predictions.json"
            span_file = f"{span_dir}/seed_{seed}_predictions.json"
            if not os.path.exists(token_file) or not os.path.exists(span_file):
            ...
            span_lookup = {r["source_text"]: r for r in span_data}
            tp_all, fp_all, fn_all, sup_all = defaultdict(int), defaultdict(int), defaultdict(int), defaultdict(int)
            export_results = []

            for record in token_data:
                text = record["source_text"]
                gt_ents = record["ground_truth"]
                token_preds = record["predictions"]
                span_record = span_lookup.get(text)
                span_preds = span_record["predictions"] if span_record else []

                ensemble_preds = ensembler.ensemble_token_primary_gapfill(token_preds, span_preds, gap_threshold)
                ...
                doc_tp, doc_fp, doc_fn, doc_sup = evaluator.calculate_counts(gt_ents, ensemble_preds)
                for k, v in doc_tp.items(): tp_all[k] += v
                for k, v in doc_fp.items(): fp_all[k] += v
                for k, v in doc_fn.items(): fn_all[k] += v
                for k, v in doc_sup.items(): sup_all[k] += v

            metrics = evaluator.compute_metrics(tp_all, fp_all, fn_all, sup_all)
            all_metrics["w_pre"].append(metrics["w_pre"])
            all_metrics["w_rec"].append(metrics["w_rec"])
            all_metrics["w_f1"].append(metrics["w_f1"])

            with open(f"{out_dir}/seed_{seed}_predictions.json", 'w', encoding='utf-8') as f:
                json.dump(export_results, f, ensure_ascii=False, indent=4)

        if not all_metrics["w_f1"]:
            continue

        avg_pre = sum(all_metrics["w_pre"]) / len(all_metrics["w_pre"])
        avg_rec = sum(all_metrics["w_rec"]) / len(all_metrics["w_rec"])
        avg_f1 = sum(all_metrics["w_f1"]) / len(all_metrics["w_f1"])
        ...
        with open(f"{out_dir}/eval_summary.json", 'w', encoding='utf-8') as f:
            json.dump(eval_summary, f, indent=2, ensure_ascii=False)

        logger.info(f"  ENSEMBLE RESULTS FOR {ensemble_name}:")
        logger.info(f"    Avg Precision: {avg_pre:.4f}")
        logger.info(f"    Avg Recall:    {avg_rec:.4f}")
        logger.info(f"    Avg F1-Score:  {avg_f1:.4f}")

        if api and not config.DEBUG_MODE:
            hub_path = f"{safe_ds}/evaluations/{safe_ens_name}"
            logger.info(f"    Pushing ensemble results to HF: {hub_path}")
            api.upload_folder(
                folder_path=out_dir,
                repo_id=TARGET_REPO,
                path_in_repo=hub_path,
...
def interactive_config():
    ...
    DATASETS = [
        "financial-pii-ready",
        "nemotron-pii-ready",
        "PII_Merged_Mapped_Dataset_Augmented",
...
def parse_args():
    ...
    parser.add_argument("--datasets", nargs='+', default=["PII_Merged_Mapped_Dataset_Augmented"], help="List of HuggingFace dataset repositories to process.")
...
def main():
    ...
    for dataset_repo in args.datasets:
        ...
        if args.eval_only:
            ...
            # Run ensemble on saved predictions
            ensemble_evaluate(safe_name, evaluator, api)
        ...
        # Run ensemble on saved predictions after all models are trained
        ensemble_evaluate(safe_name, evaluator, api)
