...
class TokenBasedModule:
    def __init__(self, model_name, unique_labels, dataset_name, use_crf=False, use_bioes=False):
        ...
        for label in unique_labels: 
            if self.use_bioes:
                # BIOES scheme: generate 4 tag variants per entity type
    ...
    def tokenize_and_align(self, example):
        ...
        # Assign -100 to all special tokens (CLS, SEP, PAD)
        ...
        for entity in example.get('privacy_mask', []):
            ...
            # Collect token indices that belong to this entity
            entity_tokens = []
            for idx, (tok_start, tok_end) in enumerate(offset_mapping):
                if labels[idx] == -100: 
                    continue
    ...
    def predict(self, text: str):
        ...
        for idx, (pred, offset) in enumerate(zip(predictions, offset_mapping)):
            ...
            if prefix == "S":
                ...
                # Handle invalid sequences (I or E tag without a preceding B tag)
                if current_entity:
                    entities.append(current_entity)
                    current_entity = None
                if prefix in ["I", "E"]:
                    current_entity = {"tag": tag_type, "start": offset[0], "end": offset[1]}
                    if prefix == "E": # Close immediately if sequence starts with E
                        entities.append(current_entity)
                        current_entity = None
