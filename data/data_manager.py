...
class PIIDataManager:
    """Manages loading and splitting PII datasets from HuggingFace Hub.
    
    Handles dataset loading, column validation, privacy mask parsing,
    train/test splitting, and optional quick-test subsampling.
    
    Args:
        dataset_name: HuggingFace dataset identifier (e.g., 'org/dataset-name').
    """
    ...
    def _parse_mask(self, mask_data):
        """Parse privacy_mask field which may be a JSON string, Python literal, or list."""
    ...
    def load_data(self) -> Tuple[List[Dict], List[Dict]]:
        """Load and process the dataset, splitting into train and test sets.
        
        Uses the 'test' or 'validation' split if available; otherwise performs
        a 90/10 random split. Respects config.RUN_QUICK_TEST and config.DEBUG_MODE
        for subsampling.
        
        Returns:
            Tuple of (train_data, test_data) as lists of record dicts.
        """
    ...
    def get_unique_labels(self) -> List[str]:
        """Extract all unique PII entity labels from both train and test data.
        
        Returns:
            Sorted list of uppercase label strings.
        """
        labels = set()
